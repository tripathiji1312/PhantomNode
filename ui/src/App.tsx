...
export default function App() {
  ...
  const connectWS = useCallback(() => {
    ...
    ws.onopen = () => {
      setConnected(true)
      setStatus('idle')
      pushEvent('Node link established', 'success')
      addToast('Connected to PhantomNode backend', 'success')
    }
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (payload?.type === 'status') {
          const msg = payload.message ?? 'Status update'
          pushEvent(msg, 'info')

          // Update progress indicators
          if (msg.includes('Encoding') || msg.includes('Generating') || msg.includes('Broadcast')) {
            setSendProgress(msg)
          }
          if (msg.includes('Activating') || msg.includes('microphone')) {
            setRxProgress(msg)
          }
          ...
          // Audio generated and played through speakers by backend
          const msg = payload.message ?? 'Audio ready'
          pushEvent(msg, 'success')
          setSendProgress('')
          setStatus('idle')
          setIsPlaying(false)
          addToast('Audio broadcast complete through speakers!', 'success')

          // Also load the WAV in the frontend player for replay
          const audioUrl = `${API_BASE}${payload.audio_url}?t=${Date.now()}`
          if (audioRef.current) {
          ...
          pushEvent(`File decoded: ${filename}`, 'file')
          addToast(`Received and decoded: ${filename}`, 'file')
          setRxProgress('')
          setStatus('idle')
    ...
    ws.onclose = () => {
      setConnected(false)
      pushEvent('Node link lost — reconnecting...', 'error')
  ...
  /* ———— Transmit File (POST /tx) ———— */
  const sendFile = useCallback(async (file: File) => {
    setStatus('sending')
    setIsPlaying(true)
    setSendProgress(`Uploading ${file.name}...`)
    pushEvent(`Preparing ${file.name} for transmission`, 'info')
    ...
    try {
      ...
      pushEvent(`${file.name} sent to encoder`, 'success')
      setSendProgress('Encoding to binary...')
      addToast(`${file.name} is being processed`, 'info')
    } catch (err) {
      setStatus('error')
      setIsPlaying(false)
      setSendProgress('')
      const msg = err instanceof Error ? err.message : 'Unknown error'
      pushEvent(`Transmission failed: ${msg}`, 'error')
      addToast(`Failed to transmit: ${msg}`, 'error')
    }
  }, [pushEvent, addToast])
  ...
  /* ———— Receive (POST /rx/start, /rx/stop) ———— */
  const startListening = useCallback(async () => {
    setStatus('listening')
    setRxProgress('Arming microphone...')
    pushEvent('Capture armed — listening for acoustic data', 'info')

    try {
      const res = await fetch(`${API_BASE}/rx/start`, { method: 'POST' })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      pushEvent('Microphone active — recording', 'success')
      setRxProgress('Recording audio...')
      addToast('Microphone is now recording', 'success')
    } catch (err) {
      setStatus('error')
      setRxProgress('')
      const msg = err instanceof Error ? err.message : 'Unknown error'
      pushEvent(`Failed to start capture: ${msg}`, 'error')
      addToast(`Capture failed: ${msg}`, 'error')
    }
  }, [pushEvent, addToast])

  const stopListening = useCallback(async () => {
    setRxProgress('Stopping capture...')
    pushEvent('Stopping capture — decoding payload', 'info')

    try {
      await fetch(`${API_BASE}/rx/stop`, { method: 'POST' })
      setStatus('idle')
      setRxProgress('Decoding audio data...')
      pushEvent('Capture stopped — awaiting decoded result', 'info')
      addToast('Recording stopped — decoding...', 'info')
    } catch {
      setStatus('idle')
      setRxProgress('')
    }
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between bg-surface-container/90 backdrop-blur-xl border-4 border-on-surface rounded-full px-4 md:px-6 py-3 shadow-neo">
          ...
          <div className="flex items-center gap-3">
            <motion.div
              ...
              className={cn(
                "flex items-center gap-2 px-4 py-2 rounded-full border-2 border-on-surface font-black text-xs uppercase tracking-widest",
                statusColor
              )}
            >
        ...
        <div className="bg-surface-container/95 backdrop-blur-xl border-4 border-on-surface rounded-full p-2 shadow-neo flex justify-around">
          {(['home', 'sender', 'receiver'] as const).map((tab) => (
            <button
              ...
              onClick={() => setPage(tab)}
              className={cn(
                "px-4 py-2 rounded-full font-black text-xs uppercase tracking-wider transition-colors",
            ...
            >
              <section className="text-center mb-12 md:mb-16 flex flex-col items-center">
                ...
                <p className="text-lg md:text-2xl font-bold text-on-surface-variant max-w-2xl bg-surface-container border-4 border-on-surface px-6 md:px-8 py-4 rounded-3xl shadow-neo-sm">
                  Transmit data through ultrasonic audio waves. Send files from one device using speakers, and receive them on another using a microphone.
                </p>
              </section>
              ...
              <section className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 w-full max-w-5xl mb-16 md:mb-24">
                ...
                >
                  ...
                  <p className="text-base md:text-xl font-bold opacity-90">Convert text or files into acoustic waveforms and broadcast them.</p>
                ...
                >
                  ...
                  <p className="text-base md:text-xl font-bold opacity-90">Listen via microphone, decode ultrasonic signals, and reconstruct data.</p>
                  ...
                  <div className="bg-surface-container border-4 border-on-surface rounded-3xl shadow-neo-sm overflow-hidden">
                    {events.slice(0, 6).map((ev, i) => (
                      <motion.div
                        ...
                        className={cn(
                          "px-6 py-4 flex items-center justify-between gap-4 border-b-2 border-on-surface/10 last:border-b-0",
                          ev.type === 'error' && 'bg-red-50',
                          ev.type === 'success' && 'bg-green-50',
                          ev.type === 'file' && 'bg-orange-50'
                        )}
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <div className={cn(
                            "w-3 h-3 rounded-full shrink-0 border-2 border-on-surface",
                        ...
                        <span className="text-xs font-bold text-on-surface-variant bg-surface-dim px-3 py-1 rounded-full shrink-0 font-mono">
                          {ev.time}
                        </span>
                      </motion.div>
                    ))}
            ...
            >
              <div className="text-center">
                ...
                <p className="text-lg md:text-2xl font-bold text-on-surface-variant bg-white border-4 border-on-surface rounded-full inline-block px-6 md:px-8 py-3 shadow-neo-sm">
                  Encode text or files → convert to audio → broadcast via speakers
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                ...
                <div className="bg-funky-cyan border-8 border-on-surface rounded-[2rem] md:rounded-[2.5rem] p-6 shadow-neo flex flex-col relative overflow-hidden">
                  ...
                  <motion.button
                    ...
                    onClick={processText}
                    disabled={isBusy || !textMessage.trim()}
                  ...
                  >
                    <Send size={24} /> {status === 'sending' ? 'Sending...' : 'Send Text'}
                  </motion.button>
                </div>
                ...
                <div className="flex flex-col gap-6 md:gap-8">
                  ...
                  <motion.div
                    whileHover={!isBusy ? { scale: 1.02, rotate: -0.5 } : {}}
                    whileTap={!isBusy ? { scale: 0.95 } : {}}
                    ...
                    className={cn(
                      "cursor-pointer text-white border-8 border-on-surface rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-12 shadow-neo flex flex-col items-center justify-center flex-1 transition-colors min-h-[200px]",
                  ...
                  <div className="bg-surface-container border-4 border-on-surface rounded-2xl md:rounded-3xl p-5 md:p-6 shadow-neo-sm">
                    <div className="flex items-center justify-between mb-4">
                      <span className="font-bold uppercase tracking-widest text-on-surface-variant text-xs md:text-sm">Transmitter Status</span>
                      <span className={cn(
                        "font-black px-3 py-1 rounded-full text-xs md:text-sm border-2 border-on-surface uppercase",
                        status === 'sending' ? 'bg-funky-orange animate-pulse' :
                        status === 'error' ? 'bg-red-400 text-white' :
                        'bg-funky-lime'
                      )}>
                        {status === 'sending' ? 'Broadcasting...' : status === 'error' ? 'Error' : 'Ready'}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                          ...
                          if (audioRef.current && audioRef.current.src) {
                            if (isPlaying) {
                              ...
                              audioRef.current.play().then(() => setIsPlaying(true)).catch(() => {
                                addToast('No audio to replay yet', 'info')
                              })
                            }
                          }
                        }}
                      ...
                      >
                        {isPlaying || status === 'sending' ? <Pause size={28} /> : <Play size={28} />}
                      </motion.button>
                      <div className="flex-1 h-10 md:h-12 bg-surface-dim rounded-full border-2 border-on-surface overflow-hidden flex items-center px-3 md:px-4 gap-0.5 md:gap-1">
                        {Array.from({ length: 20 }).map((_, i) => (
                          <motion.div
                            ...
                            className={cn(
                              "flex-1 h-1.5 md:h-2 rounded-full origin-center",
                              (isPlaying || status === 'sending') ? "bg-funky-pink" : "bg-on-surface/30"
                            )}
                          />
                        ))}
                      </div>
                ...
                <div className="bg-surface-container border-4 border-on-surface rounded-2xl md:rounded-3xl shadow-neo-sm overflow-hidden">
                  <div className="px-6 py-4 border-b-4 border-on-surface bg-funky-cyan/20 flex items-center justify-between">
                    <span className="font-black uppercase tracking-widest text-sm flex items-center gap-2">
                      <Radio size={16} /> Transmission Log
                    </span>
            ...
            >
              ...
              <div className="bg-surface-container border-8 border-on-surface rounded-[2rem] md:rounded-[3rem] p-8 md:p-12 shadow-neo min-h-[300px] md:min-h-[400px] flex flex-col items-center justify-center relative overflow-hidden">
                ...
                <div className="flex items-end justify-center gap-1 md:gap-2 h-48 md:h-64 mb-8 md:mb-12 z-10 w-full px-4 md:px-8">
                  {Array.from({ length: 40 }).map((_, i) => (
                    <motion.div
                      key={i}
                      animate={{
                        height: status === 'listening' ? [20, Math.random() * 180 + 30, 20] : 12
                      }}
                      transition={{
                      ...
                      className={cn(
                        "w-full rounded-t-lg border-2 border-on-surface border-b-0 transition-colors",
                        status === 'listening'
                ...
                <div className="w-full flex flex-col md:flex-row justify-between items-center z-10 bg-white border-4 border-on-surface rounded-2xl p-4 md:p-6 shadow-neo-sm gap-4">
                  <div className="flex items-center gap-4">
                    <div className={cn(
                      "w-4 h-4 rounded-full border-2 border-on-surface",
                    ...
                    <span className="text-lg md:text-xl font-bold uppercase tracking-widest">
                      {status === 'listening' ? 'RECORDING...' : 'Standby'}
                    </span>
                  </div>
                ...
                {receivedContent !== null && (
                  ...
                  >
                    <div className="px-6 md:px-8 py-4 border-b-4 border-on-surface flex items-center justify-between bg-white/20">
                      <div className="flex items-center gap-3">
                        <Volume2 size={24} className="text-on-surface" />
                        <span className="font-black text-lg md:text-xl uppercase tracking-wider">Decoded Output</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="font-bold text-sm bg-white/40 px-3 py-1 rounded-full border-2 border-on-surface">
                          {receivedFilename}
                        </span>
                        <motion.button
                          whileHover={{ scale: 1.1 }}
                ...
                <AnimatePresence>
                  {events.slice(0, 6).map((ev, i) => (
                    <motion.div
                      ...
                      transition={{ delay: i * 0.05 }}
                      className={cn(
                        "border-4 border-on-surface rounded-2xl md:rounded-3xl p-5 md:p-6 shadow-neo-sm flex flex-col gap-2",
                        ev.type === 'file' ? "bg-funky-orange" :
                        ev.type === 'success' ? "bg-funky-lime" :
                    ...
                    >
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="font-bold text-xs text-on-surface-variant bg-surface-dim px-3 py-1 rounded-full border-2 border-on-surface font-mono">
                          {ev.time}
                        </span>
                      </div>
